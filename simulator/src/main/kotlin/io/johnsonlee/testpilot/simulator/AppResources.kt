 ...
 * Implemented by [ResourceTableResolver] to resolve resources from the parsed resource table.
 */
interface AppResourceResolver {
 ...
 * Supports configuration-aware resource resolution via a pluggable [AppResourceResolver].
 */
class AppResources(
    val configuration: DeviceConfiguration = DeviceConfiguration.DEFAULT
) {
     ...
     */
    var resolver: AppResourceResolver? = null

    fun getString(resId: Int): String =
        resolver?.resolveString(resId)
            ?: strings[resId]
            ?: throw AppResourceNotFoundException("String resource ID #0x${resId.toString(16)}")

    fun getLayout(resId: Int): String =
        resolver?.resolveLayout(resId)
            ?: layouts[resId]
            ?: throw AppResourceNotFoundException("Layout resource ID #0x${resId.toString(16)}")

    fun getInteger(resId: Int): Int =
        resolver?.resolveInteger(resId)
            ?: throw AppResourceNotFoundException("Integer resource ID #0x${resId.toString(16)}")

    fun getBoolean(resId: Int): Boolean =
        resolver?.resolveBoolean(resId)
            ?: throw AppResourceNotFoundException("Boolean resource ID #0x${resId.toString(16)}")

    fun getColor(resId: Int): Int =
        resolver?.resolveColor(resId)
            ?: throw AppResourceNotFoundException("Color resource ID #0x${resId.toString(16)}")

    fun getDimension(resId: Int): Float =
        resolver?.resolveDimension(resId)
            ?: throw AppResourceNotFoundException("Dimension resource ID #0x${resId.toString(16)}")
...
class AppResourceNotFoundException(message: String) : RuntimeException(message)
