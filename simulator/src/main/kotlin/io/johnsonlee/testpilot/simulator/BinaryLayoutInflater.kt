 ...
 *
 * Uses real `android.*` classes from layoutlib rather than simulator shims.
 */
class BinaryLayoutInflater(
    ...
    private fun createView(tagName: String): View {
        ...
        return when (simpleName) {
            ...
            "ConstraintLayout" -> FrameLayout(context)  // AndroidX, use FrameLayout placeholder
            "CoordinatorLayout" -> FrameLayout(context)  // AndroidX, use FrameLayout placeholder
            "ScrollView" -> ScrollView(context)
            "HorizontalScrollView" -> HorizontalScrollView(context)
            "NestedScrollView" -> ScrollView(context)  // AndroidX, use ScrollView placeholder
            ...
            // Container widgets — AndroidX types use FrameLayout as placeholder
            "RecyclerView" -> FrameLayout(context)
            "ListView" -> ListView(context)
            "GridView" -> GridView(context)
            "ViewPager", "ViewPager2" -> FrameLayout(context)
            "CardView" -> FrameLayout(context)  // AndroidX, use FrameLayout placeholder

            // AppCompat variants → use framework equivalents
            ...
            "MaterialCardView" -> FrameLayout(context)
    ...
    private fun parseAttribute(view: View, attr: BinaryXmlParser.XmlAttribute) {
        ...
        when (name) {
            ...
            "textColor" -> if (view is TextView) view.setTextColor(parseColor(value))
    ...
    private fun parseLayoutParams(element: BinaryXmlParser.XmlElement, parent: ViewGroup?): ViewGroup.LayoutParams {
    ...
    private fun parseLayoutDimension(value: Any?): Int {
        return when (value) {
            -1, "match_parent", "fill_parent" -> ViewGroup.LayoutParams.MATCH_PARENT
            -2, "wrap_content" -> ViewGroup.LayoutParams.WRAP_CONTENT
            is Number -> value.toInt()
    ...
    private fun parseBackground(view: View, value: Any?) {
        when (value) {
            is Number -> view.setBackgroundColor(value.toInt())
            is String -> {
                when {
                    value.startsWith("#") -> view.setBackgroundColor(parseColor(value))
                    value.startsWith("@color/") -> view.setBackgroundColor(parseColor(value))
