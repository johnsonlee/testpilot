...
class ConfigMatcherTest {
    ...
    fun `should return only candidate when single variant`() {
        ...
        val device = DeviceConfiguration.DEFAULT
    ...
    fun `should select matching locale over default`() {
        ...
        val device = DeviceConfiguration(locale = "es")
    ...
    fun `should fall back to default when locale does not match`() {
        ...
        val device = DeviceConfiguration(locale = "ja")
    ...
    fun `should select night mode variant`() {
        ...
        val device = DeviceConfiguration(nightMode = DeviceConfiguration.NIGHT_MODE_YES)
    ...
    fun `should prefer closest density`() {
        ...
        val device = DeviceConfiguration(density = DeviceConfiguration.DENSITY_HDPI)
    ...
    fun `should eliminate contradicting orientation`() {
        ...
        val device = DeviceConfiguration(orientation = DeviceConfiguration.ORIENTATION_LANDSCAPE)
    ...
    fun `should eliminate configs requiring higher SDK`() {
        ...
        val device = DeviceConfiguration(sdkVersion = 33)
    ...
    fun `locale should take priority over density`() {
        ...
        val device = DeviceConfiguration(locale = "es", density = DeviceConfiguration.DENSITY_XHDPI)
    ...
    fun `should select matching screen layout size`() {
        val variants = candidates(
            config() to "default",
            config(screenLayout = DeviceConfiguration.SCREENLAYOUT_SIZE_NORMAL) to "normal",
            config(screenLayout = DeviceConfiguration.SCREENLAYOUT_SIZE_LARGE) to "large"
        )
        val device = DeviceConfiguration(screenLayout = DeviceConfiguration.SCREENLAYOUT_SIZE_LARGE)
    ...
    fun `should eliminate screen size larger than device`() {
        val variants = candidates(
            config() to "default",
            config(screenLayout = DeviceConfiguration.SCREENLAYOUT_SIZE_XLARGE) to "xlarge"
        )
        val device = DeviceConfiguration(screenLayout = DeviceConfiguration.SCREENLAYOUT_SIZE_NORMAL)
